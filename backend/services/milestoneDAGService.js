...
/**
 * Return a milestone's dependency list, normalized to an array.
 * `dependsOn` is optional on a milestone record — this is the single
 * place that decides what "no dependencies" means, instead of every
 * consumer repeating its own `m.dependsOn || []` fallback.
 *
 * @param {{ dependsOn?: string[] }} milestone
 * @returns {string[]}
 */
function getDependencies(milestone) {
...
export function topologicalSort(milestones) {
  ...
  for (const m of milestones) {
    ...
    for (const dep of getDependencies(m)) {
...
export function canSubmitMilestone(milestoneId, milestones, statusMap) {
  ...
  for (const dep of getDependencies(milestone)) {
