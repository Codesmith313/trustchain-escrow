...
export default function AnalyticsDashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([s, c]) => {
        setSummary(s);
        // `c` (the cohort payload) may resolve to `null` when there is no
        // cohort data yet. Every other null-check in this file uses
        // optional chaining (`?.`) / nullish coalescing (`??`) — this one
        // previously read `c.weeks` directly, which threw
        // "Cannot read properties of null (reading 'weeks')" and was
        // silently turned into a generic error state by the .catch below
        // instead of the empty-cohort state it should have been.
      ...
      .catch(err => { setError(err?.message ?? 'Failed to load analytics data'); setLoading(false); });
  }, []);
